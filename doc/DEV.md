...
2. Run `./script/dev-up.sh` script. After launching:
    ...
    - run `./script/build_js_dev.sh` in apps window to rebuild frontend

## Dev down

Just killing session won't stop `node` and `kupo`. 
To gracefully shutting down dev session you want to use following script `./script/dev-down.sh`
