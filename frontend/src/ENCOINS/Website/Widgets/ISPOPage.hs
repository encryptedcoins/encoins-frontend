module ENCOINS.Website.Widgets.ISPOPage
    ( ispoPage
    ) where
...
aboutSection = section "" "" $ do
    ...
    container "" $ do
        pClass "p-ispo" $ do
            ...
            text
                " is a novel method to distribute a protocol's utility tokens to the community using the Cardano blockchain. It ensures a high degree of decentralization of the token ownership and simultaneously supports the project's development."
        pClass "p-ispo" $ do
            text
                "Currently, we are distributing a total of 10 000 000 ENCS tokens (from the total supply of 15 000 000 ENCS) to delegators of our very own stake pool with the ticker symbol "
            el "strong" $ text "[CMIX]"
            text
                ". Our ISPO, initially started on Epoch 319 (early February 2022) as CardMix ISPO, will last until all tokens are assigned. To participate in the token distribution, users delegate their ADA to our stake pool "
            el "strong" $ text "[CMIX]"
            text
                ". The ENCS rewards are calculated for every epoch individually, so users may decide to participate for any number of epochs. Our pool is also a part of "
        ...
        pClass "p-ispo" $ do
            ...
            lnkInline
                "https://cexplorer.io/pool/pool1pmeetaqhlsdc5yj8snp8j46hprs9mzghxcl63vch95trz3xwpln"
                "cexplorer.io"
            text ", "
            lnkInline
                "https://adapools.org/pool/0ef395f417fc1b8a124784c279575708e05d8917363fa8b3172d1631"
                "adapools.org"
            text ", "
            lnkInline
                "https://pooltool.io/pool/0ef395f417fc1b8a124784c279575708e05d8917363fa8b3172d1631/epochs"
                "pooltool.io"
            text "."
            el "br" blank
            text
...
rewardsCalculatorSection = section "" "" $ do
    ...
    container "" $ do
        pClass "p-ispo" $
            text
                "Participants of the ISPO can track their accumulated rewards using the calculator below. Enter your stake key to see how many ENCS tokens you have secured so far. The information is updated every epoch (5 days). Type your stake address below:"
        calculator
        pClass "p-ispo" $ do
        ...
        pClass "p-ispo" $ do
            ...
            lnkInline
                "https://encoins-crypto.medium.com/all-ispo-info-summarized-c68e33747b77"
                "here"
            text "."
...
lotteryWinnersSection = section "" "" $ do
    ...
    container "" $ do
        pClass "p-ispo" $ do
            text
                "At the start of each epoch, we use the hashes of the first few blocks to determine our winners. Simultaneously, we publish the list of participants for the next lottery. You can find it "
        ...
        divClass "div-lottery-winners" $ do
            ...
            void $ image "fireworks.svg" "image-fireworks inverted" "120px"
        pClass "p-ispo" $
            text "The winners secure additional 4000 ENCS. Congratulations!!!"
