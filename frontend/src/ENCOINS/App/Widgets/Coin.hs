...
partitionByWalletCoinNames ::
    [AssetName] -> [TokenCacheV3] -> ([TokenCacheV3], [TokenCacheV3])
...
coinBurnWidget tokenV3@(MkTokenCacheV3 name s _) = mdo
    (elTxt, ret) <- elDynAttr "div" (mkAttrs <$> dIsSpoilerVisible) $ do
        ...
        (txt, _) <- elClass' "div" "app-text-normal" $ do
            ...
            let arrowClass =
                    bool "app-Spoiler_AssetInfo-down" "app-Spoiler_AssetInfo-up"
                        <$> dIsSpoilerVisible
            image "arrow_down_white.svg" arrowClass ""
        divClass "app-text-semibold ada-value-text" $
            text $
                coinValue s `Text.append` " ADA"
        let keyIcon = do
...
noCoinsFoundWidget =
    bool
        blank
        ( divClass "coin-entry-burn-div-no-coins" $
...
coinMintCollectionV3Widget eCoinUpdate = mdo
    ...
    dCoinsToMintV3 <-
        foldDyn
            f
            []
            (leftmost [eCoinUpdate, fmap RemoveCoin eRemoveSecret])
            >>= coinCollectionV3
    eRemoveSecret <-
        dyn (fmap (mapM coinV3MintWidget) dCoinsToMintV3)
