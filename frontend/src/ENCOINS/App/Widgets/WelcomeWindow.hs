...
welcomeWallet =
    ...
    , WelcomeItem
        ...
        ,
            ( ". Visit our Discord community at "
            , "https://discord.gg/Q3gPP87Tcw"
            , "discord.gg/Q3gPP87Tcw"
            )
        ,
            ( ". Stay up to date with the project's news on Twitter at "
            , "https://twitter.com/ENCOINS1"
            , "twitter.com/ENCOINS1"
            )
        , (".", "", "")
        ]
    ]
...
welcomeItemWidget WelcomeItem{..} eOpen = mdo
    ...
    when border $
        performEvent_ (setElementStyle elemId "border-style" "solid" <$ eOpen)
    eClose <- dialogWindow False eOpen eClose "app-WelcomeWindow" title $ do
        divClass "app-WelcomeBody" $ do
            mapM_
                ( \(t, lRef, lTitle) -> do
                    text t
                        >> if T.empty == T.strip lRef then blank else lnkInlineInverted lRef lTitle
                )
                message
        divClass "app-WelcomeBody_ButtonOkContainer" $
            btn "button-switching inverted flex-center" "" $
                text "Ok"
    ...
    when border $
        performEvent_ (setElementStyle elemId "border-style" "none" <$ eClose)
    return eClose
...
welcomeWindow key items = do
    ...
    if isSeen == Just currentHash
        then blank
            ...
            performEvent_
                ( saveJsonToStorage key currentHash
                    <$ eWelcomeClose
                )
...
wCoinsInWallet :: Text
wCoinsInWallet =
    [r|In the left column, you see the coins from your wallet.
...
wCoinsToMint :: Text
wCoinsToMint =
    [r|Here, you can add new coins to mint by entering their ADA value and clicking the "+" button (or by pressing "Enter" on the keyboard). You can remove a coin from the current transaction by clicking the "x" button.|]

wTxBalance :: Text
wTxBalance =
    [r|Here, you can see the net ADA balance of the current transaction and protocol fees. A positive balance means you are withdrawing ADA from the protocol.|]

wSend :: Text
wSend =
    [r|Once you finished building the transaction, you can press "SEND REQUEST" button to execute it.|]

wImportExport :: Text
wImportExport =
    [r|All known coins are stored locally on your device. Here, you can import the coins that are new to this device. You can also export your coins for backup or use on another device. We recommend exporting coins after each session as they are currently stored in the browser cache.|]

wDisclaimerText :: Text
wDisclaimerText =
    [r|Encrypted Coins (aka Encoins) are Cardano native tokens with encrypting redeeming values. The Protocol is comprised of free, public and open-source software including a set of immutable and autonomous smart contracts deployed on the Cardano blockchain.
...
tCoinsInWallet :: Text
tCoinsInWallet =
    [r|You are now in the transfer mode! To select the coins to transfer, check the boxes on the left.|]

tSend :: Text
tSend =
    [r|You can send coins to another user or to the ENCOINS Ledger script for use in the Ledger Mode.|]

-- Ledger welcome messages

ledger :: Text
ledger =
    [r|You are now in the Ledger mode! The ENCOINS Ledger is the on-chain script that acts as your stealth account. Sending coins to another user is as simple as sharing the minting keys off-chain. To receive a coin, import the minting key and re-mint the coin to gain the full control of it.|]

lCoinsInLedger :: Text
lCoinsInLedger =
    [r|Here, you can see the known coins that are stored on the ENCOINS Ledger. To select the coins to burn in the current transaction, check the boxes on the left.|]

lCoinsToMint :: Text
lCoinsToMint =
    [r|Here, you can add new coins to mint as in the Wallet Mode. Use the "Add Change" button to auto-balance the transaction. This creates a pure transfer on the ENCOINS Ledger without withdrawal. You can choose any wallet address as the destination when withdrawing from the ENCOINS Ledger.|]
