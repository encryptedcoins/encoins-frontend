...
welcomeWallet =
  [ WelcomeItem "welcome-tabs"
    False
    "Tabs"
    $ text [r|Welcome to ENCOINS! You can select the mode of operation from this menu.|]
  , WelcomeItem "welcome-wallet-coins"
    True
    "Coins in the Wallet"
    $ text [r|In the left column, you see the coins from your wallet.
  ...
  , WelcomeItem "welcome-coins-mint"
    True
    "Coins to mint"
    $ text [r|Here, you can add new coins to mint by entering their ADA value and clicking the "+" button (or by pressing "Enter" on the keyboard). You can remove a coin from the current transaction by clicking the "x" button.|]
  , WelcomeItem "welcome-tx-balance"
    False
    "Transaction balance"
    $ text [r|Here, you can see the net ADA balance of the current transaction and protocol fees. A positive balance means you are withdrawing ADA from the protocol.|]
  , WelcomeItem "welcome-send-req"
    True
    "Send button"
    $ text [r|Once you finished building the transaction, you can press "SEND REQUEST" button to execute it.|]
  , WelcomeItem "welcome-import-export"
    False
    "Import/Export buttons"
    $ text [r|All known coins are stored locally on your device. Here, you can import the coins that are new to this device. You can also export your coins for backup or use on another device.|]
  , WelcomeItem "welcome-read-docs"
    ...
    $ do
      ...
      text "."
  , WelcomeItem "welcome-disclaimer"
    False
    "Disclaimer"
    $ text [r|Encrypted Coins (aka Encoins) are Cardano native tokens with encrypting redeeming values. The Protocol is comprised of free, public and open-source software including a set of immutable and autonomous smart contracts deployed on the Cardano blockchain.
...
YOU AGREE THAT ANY CAUSE OF ACTION ARISING OUT OF OR RELATED TO THE SITE OR ANY PARTY MUST COMMENCE WITHIN ONE (1) YEAR AFTER THE CAUSE OF ACTION ACCRUES, OR THE CAUSE OF ACTION IS PERMANENTLY BARRED.
|]
    ]
...
welcomeTransfer =
  [ WelcomeItem "welcome-coins-transfer"
    True
    "Coins in the Wallet"
    $ text [r|You are now in the transfer mode! To select the coins to transfer, check the boxes on the left.|]
  , WelcomeItem "welcome-transfer-btns"
    True
    "Send buttons"
    $ text [r|You can send coins to another user or to the ENCOINS Ledger script for use in the Ledger Mode.|]
    ]
...
welcomeLedger =
  [ WelcomeItem "welcome-ledger"
    False
    "Ledger mode"
    $ text [r|You are now in the Ledger mode! The ENCOINS Ledger is the on-chain script that acts as your stealth account. Sending coins to another user is as simple as sharing the minting keys off-chain. To receive a coin, import the minting key and re-mint the coin to gain the full control of it.|]
  , WelcomeItem "welcome-ledger-coins"
    True
    "Coins in the Ledger"
    $ text [r|Here, you can see the known coins that are stored on the ENCOINS Ledger. To select the coins to burn in the current transaction, check the boxes on the left.|]
  , WelcomeItem "welcome-ledger-mint"
    True
    "Coins to mint"
    $ text [r|Here, you can add new coins to mint as in the Wallet Mode. Use the "Add Change" button to auto-balance the transaction. This creates a pure transfer on the ENCOINS Ledger without withdrawal. You can choose any wallet address as the destination when withdrawing from the ENCOINS Ledger.|]
    ]
...
welcomeItemWidget WelcomeItem{..} eOpen = mdo
  ...
  eClose <- dialogWindow False eOpen eClose "overflow: auto; max-width: 700px; max-height: 500px; padding-left: 30px; padding-right: 30px; padding-top: 30px; padding-bottom: 30px; top: 10px; position: absolute;" title $ do
    divClass "app-WelcomeBody" message
