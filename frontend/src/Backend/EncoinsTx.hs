...
encoinsTxWalletMode
    ...
    dUrls = mdo
        ...
        emUsedUrlDelayed <- delay 0.1 $ () <$ emUsedUrl
...
encoinsTxLedgerMode
    ...
    dUrls = mdo
        ...
        dUpdatedUrls <- updateUrls dUrls emUsedUrl
        -- Wait 0.1s until urls are updated
        emUsedUrlDelayed <- delay 0.1 $ () <$ emUsedUrl
        emUrl <- getRelayUrlE dUpdatedUrls emUsedUrlDelayed
        dmUrl <- holdDyn Nothing emUrl
        ...
        -- Wait 0.1s until pinging url is chosen
        eFireStatus <-
            delay 0.1 $ leftmost [eInitWithUrl, void eTick, eStatusFallbackWithUrl]
