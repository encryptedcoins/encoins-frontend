...
txValidity mbaseUrl mode maxAda s Wallet{..} toBurn toMint = mconcat $ zipWith f
    ...
    where
        balanceEncoins = getAda toBurn - getAda toMint
        depositBalance = getDeposit toBurn - getDeposit toMint
        balance = balanceEncoins + depositBalance
        fees     = protocolFees mode balance
        totalBalance = balance - fees
        ...
        cond12 = mode /= LedgerMode || totalBalance >= 0
        ...
        e12   = "Transaction balance should be greater or equal to zero in the Ledger Mode."
