...
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".0 "cd $TOOL_APP_PATH" C-m;
...
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".1 "cd $TOOL_APP_PATH" C-m;
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".1 "clear" C-m ;
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".1 "encoins-cloud" C-m;

tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".2 "cd $TOOL_APP_PATH" C-m;
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".2 "clear" C-m ;
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".2 "encoins --run";

tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".3 "cd $HOST_FRONT_PATH" C-m;
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".3 "clear" C-m ;
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".3 "./script/run.sh ";

tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".4 "cd $HOST_FRONT_PATH" C-m;
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".4 "clear" C-m ;
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".4 "./script/docker_run.sh" C-m ;
tmux send-keys -t "$FRONT_SESSION":"$WINDOW_APPS".4 "./script/build_js_dev.sh" C-m ;

tmux select-pane -t "$FRONT_SESSION":"$WINDOW_APPS".2
