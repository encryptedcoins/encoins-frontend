...
getBalance :: MonadWidget t m => Dynamic t Secrets -> Dynamic t Secrets -> m (Dynamic t Integer)
getBalance dCoinsBurn dCoinsMint =
  let getAda = fmap (sum . map (fromFieldElement . secretV))
  in holdUniqDyn $ zipDynWith (-) (getAda dCoinsMint) (getAda dCoinsBurn)

getBulletproofParams :: MonadWidget t m => Dynamic t Wallet -> Dynamic t (Maybe Address) -> Dynamic t Integer -> m (Dynamic t BulletproofParams)
...
getEnvironment :: MonadWidget t m => EncoinsMode -> Dynamic t Wallet -> Dynamic t (Maybe Address) -> Dynamic t Secrets -> Dynamic t Secrets
  -> m (Dynamic t Integer, Dynamic t Integer, Dynamic t BulletproofParams, Behavior t Randomness)
