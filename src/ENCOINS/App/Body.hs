...
import           ENCOINS.App.Widgets.PasswordWindow (passwordSettingsWindow, checkPasswordHash, passwordSotrageKey, resetPasswordDialog, enterPasswordWindow)
...
bodyWidget = do
  ...
  (ePassOk, eReset) <- case mPass of
    Just pass -> enterPasswordWindow pass
    _ -> (,never) <$> getPostBuild
  eResetOk <- resetPasswordDialog eReset
  widgetHold_ blank (bodyContentWidget <$ leftmost [ePassOk, eResetOk])
